...
export default function HomePage() {
  ...
  const filteredEntries = entries.filter((entry) => {
    const matchesSearchTerm = entry.url.toLowerCase().includes(searchTerm.toLowerCase()) || entry.ip.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesFilter = filter === 'todos' ||
      (filter === 'correspondente' && entry.match) ||
      (filter === 'nao-correspondente' && !entry.match) ||
      (filter === 'nao-retornado' && !entry.resolvedIp);
